...
<Steps>
  ...
  <Step title="Select evals to run">
    Attach one or more **CustomEvalConfig** IDs to the task (the evals you’ve already created for the project). The task runs each selected eval on every span it processes. For evals that need an input (e.g. Bias Detection), configure the **input key** to a span attribute path (e.g. `llm.output_messages.0.message.content`) so the eval reads the right field from each span. See [built-in evals](/future-agi/get-started/evaluation/builtin-evals/overview) for supported evaluations and their inputs.
  </Step>
...
- **Span attributes** — Spans store data in key-value form (e.g. `llm.output_messages.0.message.content`). When an eval needs an input, you point it to one of these attribute paths. See [spans](/product/observe/tracing/concept/spans) and [span attributes](/product/observe/tracing/concept/spans#span-attributes) for the schema.
