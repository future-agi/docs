...
<Steps>
  ...
  <Step title="View sessions in Observe">
    In the Observe UI, open the project and go to the Sessions view. You can filter by time range, see a list of sessions (with duration and metrics), open a session to see its traces, and use **View Trace** to see span-level detail and [evaluation](/product/observe/evals) results.
  </Step>
</Steps>

<Note>
  For more on setting `session.id` with Trace AI helpers and context, see the [manual tracing guide](/future-agi/get-started/observability/manual-tracing/set-session-user-id).
</Note>
