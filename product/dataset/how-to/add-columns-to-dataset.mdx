...
- **Dynamic columns** – <Tooltip tip="Dynamic columns are generated on-the-fly and contain data derived from other columns or external operations." cta="Learn more about dynamic columns" href="/future-agi/get-started/dataset/concept/dynamic-column">Values are computed or fetched</Tooltip> when you need them (e.g. from an LLM prompt, vector DB, API, custom code, or from existing columns). You configure the type, test, then create; the system fills the column row by row.
...
<Tabs sync={false}>
  ...
  <Tab title="Dynamic Column" icon="wand-magic-sparkles">
    ...
    <Tip>
      [Run Prompt in Dataset](/product/dataset/how-to/run-prompt-in-dataset) has the full walkthrough.
    </Tip>
  </Tab>
