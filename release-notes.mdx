...
rss: true
---
<Update label="Week of 2026-03-16">
...
<Update label="Week of 2026-02-27">

## Features

- **evaluate() — Unified Evaluation API:**
  New single-function entrypoint for all evaluations. Automatically routes between local heuristics (`<1ms`), cloud Turing models, and LLM-as-Judge based on parameters. Replaces manual engine selection with smart auto-routing.

- **Multimodal LLM Judge (Images & Audio):**
  Pass `image_url`, `input_image_url`, `output_image_url`, or `audio_url` to evaluate visual and audio content. Works with Gemini, GPT-4o, and other vision/audio models via LiteLLM. Verify product image descriptions, compare screenshots, check transcription accuracy.

- **Auto-Generate Grading Criteria:**
  Set `generate_prompt=True` to describe what you want to evaluate in plain English. The LLM generates detailed, specific grading criteria with score definitions for 1.0/0.5/0.0. Results are cached per session.

- **LLM Augmentation (augment=True):**
  Combine fast local heuristics with LLM refinement. Run the local metric first, then pass its analysis to an LLM for nuanced scoring. Supported for faithfulness, hallucination_score, task_completion, and more.

- **Feedback Loop System:**
  Submit corrections when the judge is wrong. Future evaluations retrieve similar past corrections as few-shot examples via ChromaDB vector search. Includes threshold calibration from developer feedback.

- **72+ Local Evaluation Metrics:**
  String checks, JSON validation, similarity scoring, hallucination detection (NLI-based), RAG evaluation, function calling accuracy, agent trajectory analysis, structured output validation, and security guardrails — all running locally with no API keys.

- **OpenTelemetry Integration:**
  Wire evaluation scores into existing observability stacks. Every evaluate() call can emit OTEL spans with gen_ai.evaluation.* attributes for Jaeger, Datadog, and Grafana.

- **Streaming Evaluation:**
  Monitor LLM output token-by-token. Detect toxicity, PII, or quality drops mid-generation and stop the stream before bad content reaches users.

## Bugs/Improvements

- **Poetry to uv Migration:**
  Migrated build system from Poetry to uv for 10x faster dependency resolution and installs.

- **Improved SDK Documentation:**
  Added comprehensive evaluate() API reference, evaluation patterns guide, and AI Evaluation SDK cookbooks to docs.

</Update>
<Update label="Week of 2026-02-25">
...
- **More Output Formats in Prompt Workbench:**  
  Prompt Workbench now supports image and audio output types in addition to text, allowing cross-format prompt testing without leaving the workbench.
...
<Update label="Week of 2026-02-13">
