...
<Update label="Week of 05-09-2025">
  #### What's New

  **Features**
  - **Insights now available in trace detail drawer:** View in-depth insights of traces including score-based breakdown of performance under different metrics, affected spans and the recommendations to fix them
  - **Add Annotation Summary Page in Dataset:** Introduced a comprehensive summary page that provides insights into the annotation process. The page displays agreement levels between annotators for categorical, numeric or text annotations, average annotation time per annotator, estimated completion time, and other quality metrics
  - **Access prompts and prompt templates within a folder-based file system. Templates behave like regular prompts and can be created from existing ones. They can be saved, and reused, enabling consistent usage across different workflows
</Update>
