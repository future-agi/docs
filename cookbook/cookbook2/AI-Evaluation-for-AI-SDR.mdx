...
```
```plaintext
Enter your FI API Key: ··········
Enter your FI Secret Key: ··········
Evaluator client initialized successfully!
```

## 4. Defining Deterministic Test Case
Definig custom deterministic test class that is tailored to our use case.
...
## 6. Evaluating AI Generated Openers Using Deterministic Eval
Below code will create test case for each judging criteria using derterministic eval.
...
for criterion, description in JUDGING_CRITERIA.items():
  ...
  deterministic_eval = Deterministic(config={
      "multi_choice": False,
      "choices": ["Good", "Poor"],
      "rule_prompt": f"opener : {{{{input_key1}}}}, combined_posts : {{{{input_key2}}}}, value_proposition: {{{{input_key3}}}}. Given the combined_posts and value_proposition, {description}",
      "input": {
          "input_key1": "opener",
  ...
  for index, row in dataset.iterrows():
    test_case_1 = DeterministicTestCase(
        opener=row['opener_1'],
        combined_posts=row['combined_posts'],
        value_proposition=row['value_proposition']
    )
    result_1 = evaluator.evaluate([deterministic_eval], [test_case_1])
    option_1 = result_1.eval_results[0].data[0]
    results_1.append(option_1)

  results_2 = []
  for index, row in dataset.iterrows():
    test_case_2 = DeterministicTestCase(
        opener=row['opener_2'],
        combined_posts=row['combined_posts'],
        value_proposition=row['value_proposition']

    )
    result_2 = evaluator.evaluate([deterministic_eval], [test_case_2])
    option_2 = result_2.eval_results[0].data[0]
