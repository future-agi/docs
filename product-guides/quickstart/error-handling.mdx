...
```python
from fi.utils.errors import InvalidAdditionalHeaders

try:
    logger = Client(api_key=api_key, secret_key=secret_key, uri=base_url, additional_headers={"Invalid-Header": "value"})
...
```python
from fi.utils.errors import InvalidValueType

tags = "This should be a dict"

try:
    logger.log(model_id="model_id", model_type=ModelTypes.GENERATIVE_LLM, environment=Environments.PRODUCTION, tags=tags)
...
```python
from fi.utils.errors import InvalidSupportedType

invalid_model_type = "UNSUPPORTED_MODEL"

try:
    logger.log(model_id="model_id", model_type=invalid_model_type, environment=Environments.PRODUCTION)
...
```python
from fi.utils.errors import MissingRequiredKey

try:
    logger.log(model_type=ModelTypes.GENERATIVE_LLM, environment=Environments.PRODUCTION)
...
```python
from fi.utils.errors import InvalidVectorLength

invalid_vector = [1, 2, 3]  # Assuming the required length is different

try:
    logger.log(model_id="model_id", model_type=ModelTypes.GENERATIVE_LLM, environment=Environments.PRODUCTION, embedding_vector=invalid_vector)
