...
A knowledge base rarely stays static: policies get revised, new documents arrive, and old ones get pulled. Continuing with the `support-policies` knowledge base from [Create a knowledge base](/docs/knowledge-base/guides/create-knowledge-base), this guide covers the four ways you keep it current: adding more documents, renaming it, removing individual files, and deleting it altogether.

Open `support-policies` from the knowledge base list (`/dashboard/knowledge`) by clicking its row to reach its detail screen at `/dashboard/knowledge/:knowledgeId`. Adding documents, renaming, and removing files all happen from that detail screen; deleting the knowledge base itself happens from the list instead.
...
Everything you add still counts against the knowledge base's 1 GB total, and a file whose name already exists in `support-policies` is refused at submission rather than added as a duplicate.
