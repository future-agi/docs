---
title: "Overview"
description: "Where the platform reads your indexed documents, and where to go next."
---

## What is Knowledge Base?

A knowledge base is a named set of documents you upload and index once.

Where it's used: [synthetic data generation](/docs/dataset/concept/synthetic-data), [agent-type evaluations](/docs/evaluation), and a [Simulation agent definition](/docs/simulation/concepts/agent-definition).

## Dive deeper

<CardGroup cols={2}>
  <Card
    title="Understanding Knowledge Base"
    icon="brain"
    href="/docs/knowledge-base/concepts/understanding-knowledge-base"
  >
    How indexing works, why it isn't live retrieval, and the size and file-type limits
  </Card>
  <Card
    title="Create a knowledge base"
    icon="plus"
    href="/docs/knowledge-base/guides/create-knowledge-base"
  >
    Name a knowledge base and upload your first documents
  </Card>
  <Card
    title="Update a knowledge base"
    icon="pen-to-square"
    href="/docs/knowledge-base/guides/update-knowledge-base"
  >
    Add or remove documents, or delete a knowledge base entirely
  </Card>
  <Card
    title="Manage with the SDK"
    icon="code"
    href="/docs/knowledge-base/guides/manage-with-the-sdk"
  >
    Create, update, and delete knowledge bases from code, same as in the UI
  </Card>
</CardGroup>
