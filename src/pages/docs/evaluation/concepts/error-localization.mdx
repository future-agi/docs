...
Turn it on with the Error Localization checkbox when you configure an eval. Set it on the eval template and every run inherits it. Set it on a single run to turn it on even when the template has it off, though a run can't turn it off once the template has it on. See [Running Evaluations](/docs/evaluation/guides/running-evaluations) for the full run flow.
