...
An eval template is the definition of what to measure. It contains the criteria the evaluator model will apply to each response and specifies what kind of result to return. You create a template once and reuse it across any dataset, simulation, experiment, or SDK call.
...
See [Create custom evals](/docs/evaluation/guides/custom-evals) for how to write your own.
...
Every result also includes a **reason**: a plain-language explanation of why the evaluator model assigned that result. This makes it possible to understand failures without reviewing each response manually.
...
- [Create custom evals](/docs/evaluation/guides/custom-evals): Write your own criteria and rule prompts.
- [Eval types](/docs/evaluation/concepts/eval-types): LLM as Judge, Deterministic, Statistical Metric, and LLM as Ranker.
- [Evaluator models](/docs/evaluation/concepts/evaluator-models): How the model applies a template to produce a result.
- [Output types](/docs/evaluation/concepts/output-types): What the output of an eval run looks like
