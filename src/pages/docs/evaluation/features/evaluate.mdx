...
<Tabs sync={false}>
  ...
  <Tab title="Using SDK" icon="code">
    <Note>
    Some evals can run without an API key using the standalone `evaluate()` function — great for local metrics like `contains`, `faithfulness`, and LLM-as-judge. See the SDK reference for details.
    </Note>
    <Steps>
