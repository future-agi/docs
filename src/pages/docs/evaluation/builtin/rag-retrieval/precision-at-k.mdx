...
- [**Recall@K**](/docs/evaluation/builtin/rag-retrieval/recall-at-k): Precision@K measures retrieval quality (how clean the results are), while Recall@K measures retrieval coverage (how many relevant items were found). Optimizing one often trades off against the other.
- [**NDCG@K**](/docs/evaluation/builtin/rag-retrieval/ndcg-at-k): NDCG@K considers both relevance and ranking position, while Precision@K treats all positions equally within the top K.
- [**Chunk Utilization**](/docs/evaluation/builtin/rag-retrieval/chunk-utilization): Precision@K evaluates retrieval quality before generation, while Chunk Utilization measures how well the generator actually uses the retrieved chunks.
