...
- [**Recall@K**](/docs/evaluation/builtin/rag-retrieval/recall-at-k): Recall@K only checks if relevant chunks appear in the top K, regardless of position. NDCG@K also rewards placing them higher in the ranking.
- [**Precision@K**](/docs/evaluation/builtin/rag-retrieval/precision-at-k): Precision@K measures the fraction of relevant results without considering order, while NDCG@K penalizes relevant results that appear late.
- [**MRR**](/docs/evaluation/builtin/rag-retrieval/mrr): MRR only cares about where the first relevant chunk appears, while NDCG@K evaluates the ranking quality across all relevant chunks.
