...
- **Output Type**, what it returns: pass or fail, or a percentage score, covered in [Output types](/docs/evaluation/reference/output-types)
...
An eval usually carries more than one, so `groundedness` is tagged both RAG and Retrieval Systems, and `prompt_injection` is tagged Red Teaming.
