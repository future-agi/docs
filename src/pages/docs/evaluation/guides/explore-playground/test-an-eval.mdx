...
The panel gives the eval its inputs one of four ways, each a tab at the top:

- **Custom**, paste your own test data as JSON, one value for each variable the eval needs, the quickest way to try a one-off case
...
*In Custom, the eval's variables are the JSON keys, so you only fill in their values*
