...
This cookbook walks through that red-team pipeline for EduBright's tutoring bot using FutureAGI's safety stack: crafting adversarial prompts, screening them with **Protect** input guardrails (using `prompt_injection` and `toxicity` metrics to catch jailbreaks and harmful content), evaluating the bot's responses with **custom Evals** on a red-team rubric, hardening the system prompt based on the findings, and adding **Protect** output guardrails as defense-in-depth. For a broader look at how Protect fits into a full guardrail pipeline, see [Secure AI Evals and Guardrails](/docs/cookbook/use-cases/secure-ai-evals-guardrails).
...
See [Protect Guardrails](/docs/cookbook/quickstart/protect-guardrails) for the full list of Protect metrics and the Protect Flash option for high-volume pipelines. Note that the `security` metric is an alias for `prompt_injection`; both route to the same detection engine.
...
<CardGroup cols={3}>
  ...
  <Card title="Protect Guardrails" icon="gauge" href="/docs/cookbook/quickstart/protect-guardrails">
    Full Protect metric reference and Protect Flash for high-volume pipelines
  </Card>
  <Card title="End-to-End Agent Testing" icon="flask" href="/docs/cookbook/use-cases/end-to-end-agent-testing">
    Test with diverse personas via simulation
  </Card>
</CardGroup>
