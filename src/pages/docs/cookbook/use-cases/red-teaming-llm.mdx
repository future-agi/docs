...
<Warning>
**MEDIA TODO — Screen Recording:** Navigate to **app.futureagi.com → Evals → Create Evaluation**. Record creating both custom evals in sequence: (1) `educational_safety` — Use Future AGI Agents, `turing_small`, Pass/Fail, paste the safety Rule Prompt (scroll to show PASS/FAIL criteria), click Create; (2) `pedagogical_guidance` — same steps but select **Percentage** as the output type (highlight this differs from Pass/Fail), paste the 40/30/30 scoring rubric prompt, click Create; (3) the Evals list showing both `educational_safety` and `pedagogical_guidance` visible together. End recording after both evals are confirmed in the list. Save as `step-4-create-safety-evals.mp4`.
</Warning>
