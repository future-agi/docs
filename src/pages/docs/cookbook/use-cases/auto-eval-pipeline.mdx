...
If the auto-detected recommendations don't match your needs, start from a pre-built template instead:
...
Since ReturnRight is a RAG support bot, the `rag_system` template is a better starting point. Let's use it:
...
The template gives you a solid baseline, but ReturnRight has domain-specific risks that generic thresholds won't catch. Time to tune.

</Step>
<Step title="Tune the pipeline for ReturnRight's needs">

The RAG template includes `faithfulness`, `groundedness`, and `answer_relevancy` out of the box. ReturnRight can't afford hallucinated prices, so tighten the faithfulness threshold and add `completeness` to catch one-liner answers.
...
pipeline.set_threshold("faithfulness", 0.85).add(
    EvalConfig(name="completeness", threshold=0.7)
).disable("groundedness")
```
...
After running the pipeline, aggregate results to see where failures cluster. If faithfulness keeps failing, your retrieval is pulling the wrong documents. If relevancy fails, your query routing needs work.
...
Once you've fixed the issues, re-run the pipeline to verify improvements. Now that you know what breaks and why, lock it down so it stays fixed.
...
ReturnRight's support bot now has quality gates that would have caught the $1,899 pricing error and the leaked email before either reached a customer. The next time a model update introduces a regression, the pipeline flags it in CI instead of letting it ship to production.
