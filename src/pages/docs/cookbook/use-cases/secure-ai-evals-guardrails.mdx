...
Here is the difference Protect makes. On the left, both attacks succeed. On the right, Protect blocks them before the damage is done:

![Before and after: Without guardrails both attacks succeed. With FutureAGI Protect, prompt injection is blocked on input and PII is caught on output.](https://fi-cookbook-assets.s3.ap-south-1.amazonaws.com/use-cases/secure-ai-evals-guardrails/guardrail-before-after.png)

The rest of this cookbook adds real enforcement, layer by layer: input screening (Step 2), output screening (Step 3), bias detection (Step 4), quality scoring (Step 5), and the full pipeline (Step 6).
