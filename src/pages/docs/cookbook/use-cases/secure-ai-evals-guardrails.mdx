---
title: "Add Guardrails and Quality Scoring to Your Chatbot"
description: "Screen chatbot inputs for prompt injection and harmful content, catch PII in outputs, and score response quality for completeness and factual accuracy."
---
...
You have a chatbot that takes user input and generates responses. Some of those inputs will be prompt injection attempts. Some responses will accidentally leak PII. And even when nothing is blocked, you still need to know whether the response was actually good.

This cookbook shows you how to screen every input, screen every output, and score response quality, all wired into a single pipeline you can drop into any chatbot.
...
<Steps>
<Step title="Set up the chatbot you want to protect">
...
<Step title="Detect biased responses">
...
<Step title="Score response quality with evals">
...
<Step title="Wire it all into a single pipeline">
...
<Step title="Log safety events for monitoring">
