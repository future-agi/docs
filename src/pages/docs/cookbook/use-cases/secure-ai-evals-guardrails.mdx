...
This cookbook builds that pipeline for WealthBridge's chatbot. You will wire together input screening, output screening, bias detection, and quality scoring into one function that runs on every conversation. By the end, prompt injection attempts never reach the model, PII never reaches the user, and every response that does get through has a quality score attached to it.
...
<Note>
Exact outputs may vary depending on model version and backend configuration. Focus on the relative patterns (which cases pass vs fail) rather than exact values.
</Note>
...
Sample output (your scores may differ):
```
ID           Metric                 Score    Passed
-------------------------------------------------------
qa_001       completeness           1.00     PASS
qa_001       context_adherence       1.00     PASS

qa_002       completeness           0.00     FAIL
qa_002       context_adherence       1.00     PASS

qa_003       completeness           1.00     PASS
qa_003       context_adherence       1.00     PASS
```
