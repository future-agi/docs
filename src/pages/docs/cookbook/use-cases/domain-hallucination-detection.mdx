...
kb_client.create_kb(name="medisafe-drug-formulary")
kb_id = str(kb_client.kb.id)
print(f"Using KB: {kb_client.kb.name} (ID: {kb_id})")

# Run groundedness with KB
...
# Run custom domain eval with KB
dataset.add_evaluation(
    name="domain-accuracy-check",
    eval_template="domain_accuracy",
    required_keys_to_column_names={
        "input": "input",
        "output": "output",
        "context": "context",
...
2. Once evaluations complete, you'll see new columns for each eval: `groundedness-check`, `factual-accuracy-check`, `completeness-check`, `domain-accuracy-check`
