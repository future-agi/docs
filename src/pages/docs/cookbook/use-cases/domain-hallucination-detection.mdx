...
for i, test in enumerate(test_cases):
    ...
    # Does the response stay within the source context?
    accuracy = evaluate(
        "context_adherence",
        output=test["output"],
        context=test["context"],
...
For the correct responses (test cases 1, 4, 5), all metrics typically pass. For the hallucinated responses, the interesting part is *which* metrics flag the problem. That tells you the root cause.
...
The pattern to look for in each hallucinated case:

- **Test case 2 (invented number):** `context_relevance` typically passes (right document fetched), but `groundedness` and `context_adherence` are likely to fail. This is a generation problem.
- **Test case 3 (omitted information):** `context_relevance` typically passes. `completeness` is likely to flag the missing interaction. `context_adherence` is likely to catch the false claim that "no significant interactions" exist.
...
2. Once evaluations complete, you'll see new columns: `groundedness-check`, `context-adherence-check`, `completeness-check`
3. Each row shows a Pass/Fail verdict or a percentage score with color coding (green for high, red for low)
4. Click on any row to expand the evaluation reasons and see why a particular score was given
