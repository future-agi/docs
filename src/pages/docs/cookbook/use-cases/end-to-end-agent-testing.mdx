...
1. **Create Agent Definition:** Go to **Simulate** → **Agent Definition** → **Create agent definition**. The 3-step wizard asks for:
   - **Basic Info:** Agent type = `Chat`, name = `acme-sdr`
   - **Configuration:** Model = `gpt-4o-mini`
   - **Behaviour:** Paste the v1 system prompt: `You are a sales assistant for Acme Inc. Help leads learn about our product and book demos.`, add a commit message, and click **Create**

2. **Create Scenarios:** Go to **Simulate** → **Scenarios** → **Create New Scenario**. Select **Workflow builder**, then fill in:
   - **Scenario Name:** `acme-sdr-leads`
   - **Description:** `Inbound leads asking about Acme Inc marketing analytics: pricing, features, objections, demo booking, and edge cases.`
   - **Choose source:** Select `acme-sdr` (Agent Definition), version `v1`
   - **No. of scenarios:** `20`
   - Leave the **Add by default** toggle on under **Persona** to auto-attach built-in personas, then click **Create**

   <Tip>Want more targeted stress-testing? Create custom personas (e.g., an aggressive negotiator or a confused non-technical buyer) via **Simulate** → **Personas** → **Create your own persona**. See the [Chat Simulation cookbook](/docs/cookbook/quickstart/chat-simulation-personas) for the full persona creation walkthrough: personality, communication style, tone, typo levels, and more.</Tip>

3. **Configure and Run Simulation:** Go to **Simulate** → **Run Simulation** → **Create a Simulation**. The 4-step wizard:
   - **Step 1: Details:** Simulation name = `acme-sdr-v1`, select `acme-sdr` agent definition, version `v1`
   - **Step 2: Scenarios:** Select the `acme-sdr-leads` scenario
...
<Note>For the full walkthrough of agent definitions, custom personas, scenario types, and simulation configuration, see [Chat Simulation: Run Multi-Persona Conversations via SDK](/docs/cookbook/quickstart/chat-simulation-personas).</Note>
...
The platform runs all 20 conversations, each with its own persona and scenario. Every conversation is traced (Step 3) and evaluated against all 10 metrics from the Conversational agent evaluation group. Results appear in the dashboard once all conversations complete.
...
You'll see aggregate scores across all 20 conversations for each of the 10 evaluation metrics, including conversation quality, context retention, query handling, loop detection, escalation handling, and prompt conformance.
...
<Note>
**Deep dive:** See [Evaluate Customer Agent Conversations](/docs/cookbook/quickstart/conversation-eval) for all 10 metrics in the Conversational agent evaluation group, individual metric examples, prompt conformance scoring, and how to run the full eval group from the dashboard on any dataset.
</Note>
...
And here's the thing: this isn't a one-time setup. When Agent Compass flags a new failure pattern next month (lead behavior changes, your product changes, the world changes), you know the drill: diagnose → optimize → re-test → promote. The loop runs itself.
...
- **Evaluation** scored every conversation across 10 quality metrics automatically
...
<CardGroup cols={4}>
  ...
  <Card title="Conversation Eval" icon="chart-bar" href="/docs/cookbook/quickstart/conversation-eval">
    All 10 metrics, prompt conformance, eval groups, diagnostic sweeps
  </Card>
