...
The SDK runs all 20 scenarios through your agent callback, sending each simulated user message and collecting your agent's responses. Results and eval scores appear in the dashboard under **Simulate** once processing completes (usually 2-5 minutes).
...
We ran the Conversational Agent evaluation group (10 evals) across all 20 simulated calls. The critical analysis surfaced 4 failure clusters:
...
Here is what Fix My Agent surfaced from our 20 conversations:
...
**System-level insights:** Average response latency was 3,872ms (above the 3,000ms threshold for natural conversation), and 9 out of 20 conversations had low CSAT scores. The recommendation: upgrade the model or implement streaming to reduce perceived latency.
...
To fully close the loop, re-run the simulation with v2 against the same 20 scenarios and check the critical analysis feed for remaining failure clusters. Any evals that held at 0.50 (like loop detection or clarification seeking) may need a follow-up optimization round targeting those specific patterns.
