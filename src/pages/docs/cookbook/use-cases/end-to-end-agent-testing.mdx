...
<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `sales-assistant` → click the **Feed** tab. Record: (1) the Feed tab loading with error clusters visible, (2) clicking into one cluster to expand it showing the Recommendation, Immediate Fix, Root Cause, and Evidence sections, (3) clicking a span link in Evidence to navigate to the failing trace. End recording after the evidence span is visible. Save as `step-6-agent-compass-feed.mp4`.
</Warning>
...
prompt.create_new_version(
    ...
    commit_message="v2: adds qualification framework, objection handling, escalation rules",
)
...
Once verified, commit v2 and promote it to production:

```python
from fi.prompt import Prompt

# Commit the draft before assigning a label
prompt = Prompt.get_template_by_name(name="sales-assistant")
prompt.commit_current_version(message="v2: qualification framework, objection handling, escalation rules")
...
<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `sales-assistant`. Record: (1) clicking the **Configure** gear icon, setting Agent Compass sampling to `20%`, and saving; (2) switching to the **Alerts** tab → clicking **Create Alert**, selecting alert type **LLM response time**, setting warning to `5` seconds and critical to `10` seconds, selecting a notification channel, and clicking **Create**; (3) repeating for the error rate and token budget alerts. End recording after all three alerts appear in the list. Save as `step-10-monitoring-setup.mp4`.
</Warning>
...
<CardGroup cols={4}>
  <Card title="Chat Simulation" icon="comments" href="/docs/cookbook/quickstart/chat-simulation-personas">
    Custom personas, scenario builders, tool-calling simulation
  </Card>
  <Card title="Conversation Eval" icon="chart-bar" href="/docs/cookbook/quickstart/conversation-eval">
    All 10 metrics, prompt conformance, diagnostic sweeps
  </Card>
  ...
  <Card title="All Quickstarts" icon="rocket" href="/docs/cookbook">
    Feature-by-feature guides for every capability
  </Card>
</CardGroup>
