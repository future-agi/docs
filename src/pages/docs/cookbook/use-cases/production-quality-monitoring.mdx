...
In **Tracing**, click any trace and expand the span detail panel. Switch to the **Evals** tab to see scores for `completeness_check`, `factual_accuracy_check`, and `context_relevance_check`. The eval columns also appear in the main trace table, so you can filter for low-scoring responses directly.

<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `my-production-app`. Record: (1) clicking into a trace, switching to the **Evals** tab, and showing the eval rows with scores and reasoning; (2) returning to the trace list, clicking the filter icon, selecting **Evaluation Metrics** → `factual_accuracy_check`, setting a threshold, and showing the filtered results. Save as `step-2-inline-evals-dashboard.mp4`.
</Warning>
...
<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `my-production-app` → click the **Alerts** tab → click **Create Alerts**. Record: (1) selecting alert type **LLM response time**, (2) setting warning to `3000` ms and critical to `5000` ms, (3) setting interval to **5 minute interval**, (4) adding a notification channel, (5) clicking **Create**. Save as `step-3-create-alerts.mp4`.
</Warning>
...
<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `my-production-app` → click the **Configure** gear icon. Record: (1) the Configure panel opening, (2) setting Agent Compass sampling to `100%`, (3) clicking save. Save as `step-4-configure-agent-compass.mp4`.
</Warning>
...
<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `my-production-app` → click the **Feed** tab. Record: (1) scrolling through error clusters, (2) clicking into one cluster to show the Recommendation, Root Cause, and Evidence sections, (3) clicking a span link to navigate to the failing trace. Save as `step-4-agent-compass-feed.mp4`.
</Warning>
...
<CardGroup cols={4}>
  <Card title="Inline Evals in Tracing" icon="chart-line" href="/docs/cookbook/quickstart/inline-evals-tracing">
    Full inline eval workflow and dashboard filtering
  </Card>
  <Card title="Monitoring & Alerts" icon="bell" href="/docs/cookbook/quickstart/monitoring-alerts">
    Charts, thresholds, and notification setup
  </Card>
  ...
  <Card title="Agent Compass" icon="compass" href="/docs/cookbook/quickstart/agent-compass-debug">
    Error clustering and fix-and-verify workflow
  </Card>
</CardGroup>
