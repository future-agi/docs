...
<Warning>
**MEDIA TODO — Screen Recording:** Navigate to **Tracing** → select `homekey-assistant`. Record: (1) clicking into a trace, switching to the **Evals** tab, and showing the `completeness_check`, `factual_accuracy_check`, and `context_relevance_check` rows with scores and reasoning; (2) returning to the trace list, clicking the filter icon, selecting **Evaluation Metrics** → `factual_accuracy_check`, setting a threshold below `0.7`, and showing the filtered results. End recording after the filtered list is visible. Save as `step-2-inline-evals-dashboard.mp4`.
</Warning>
...
<Warning>
**MEDIA TODO — Screen Recording:** Navigate to **Tracing** → select `homekey-assistant` → click the **Alerts** tab → click **Create Alerts**. Record: (1) the alert creation form opening, (2) selecting alert type **LLM response time**, (3) setting warning to `3000` ms and critical to `5000` ms, (4) setting interval to **5 minute interval**, (5) adding a notification channel, (6) clicking **Create**. Repeat for the second alert (LLM API failure rates). End recording after at least two alerts appear in the alerts list. Save as `step-4-create-alerts.mp4`.
</Warning>
...
<Warning>
**MEDIA TODO — Screen Recording:** Navigate to **Tracing** → select `homekey-assistant` → click the **Configure** gear icon. Record: (1) the Configure panel opening, (2) locating the Agent Compass sampling setting, (3) setting it to `100%`, (4) clicking save or confirm. End recording after the setting is saved successfully. Save as `step-5-configure-agent-compass.mp4`.
</Warning>
...
<Warning>
**MEDIA TODO — Screen Recording:** Navigate to **Tracing** → select `homekey-assistant` → click the **Feed** tab. Record: (1) the Feed tab loading with error clusters visible, (2) scrolling through several clusters to show pattern names, event counts, and user impact, (3) clicking into one cluster to expand it showing the Recommendation, Immediate Fix, Root Cause, and Evidence sections, (4) clicking a span link in the Evidence section to navigate to the specific failing trace. End recording after the evidence span is shown. Save as `step-6-agent-compass-feed.mp4`.
</Warning>
