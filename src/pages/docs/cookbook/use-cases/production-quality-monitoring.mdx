...
<Warning>
**MEDIA TODO: Screen Recording.** Navigate to **Tracing** → select `my-production-app`. Record: (1) clicking into a trace, switching to the **Evals** tab, and showing the eval rows with scores and reasoning; (2) returning to the trace list, clicking the filter icon, selecting **Evaluation Metrics** → `context_adherence_check`, setting a threshold, and showing the filtered results. Save as `step-2-inline-evals-dashboard.mp4`.
</Warning>
