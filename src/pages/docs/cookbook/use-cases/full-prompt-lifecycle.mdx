...
This cookbook builds that workflow for NovaCorp's HR onboarding assistant. You will store the current prompt as v1, measure its baseline performance, run an optimizer to generate v2, A/B test both versions in a structured experiment, promote the winner, and set up a one-line rollback. By the end, you will have a repeatable, evidence-based process for iterating on any prompt in production.
