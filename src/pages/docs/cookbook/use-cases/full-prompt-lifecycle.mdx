...
This cookbook builds that workflow for NovaCorp's HR onboarding assistant using FutureAGI's prompt lifecycle stack. You will store the current prompt in **Prompt Management** as v1, measure its baseline with **Evals**, run **Optimize** (`MetaPromptOptimizer`) to generate v2, A/B test both versions in **Experimentation**, promote the winner with a single label change, and roll back in one line if anything goes wrong. By the end, you will have a repeatable, evidence-based process for iterating on any prompt in production.
