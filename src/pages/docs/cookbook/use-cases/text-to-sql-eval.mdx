...
The `text_to_sql` metric validates SQL syntax and basic intent alignment. It may not catch subtle logic errors like case 5, where the question asks for "all orders" but the SQL filters to completed only. That is where `ground_truth_match` and execution testing (later steps) come in as additional safety nets.

This metric is your first line of defense, and importantly, it doesn't need a reference query. You can use it even when you don't have gold-standard SQL to compare against. But intent validation alone can't tell you if the SQL produces the right numbers. For that, you need to compare against a known-good query.
