...
You instrumented an app, a trace showed up, and you called it done. Then someone asks what last month cost per customer, and the answer is not there.

The failures below all produce a trace list that looks populated:

- **Cost and token columns are empty.** Nothing computes cost for you. It is a number you send.
- **One request appears as three traces.** A thread hand-off, a stream, or a job with no entry point split it, and each piece looks like a valid trace on its own.
...
Reading the dashboard cannot separate these from a correct integration. That is the actual problem: verification by eyeball.

## The Solution: Six Steps and Ten Gates

Six steps, each closing named gates, and one checker that decides the result. `fi_verify.py` reads a local capture of your spans for nine of the gates, and two delivery receipts for the tenth, because a local capture says nothing about whether anything reached the collector.

It exits `0`, or it prints the gate that failed and why.
...
│   ├── fi_verify.py           # the checker, downloaded in Step 1     all ten
...
└── .gitignore                 # + .fi_verify/
  ...
  one request  ──►  CHAIN root                        G3  one root, no orphans
                    ├── LLM   gpt-4o-mini             G4  typed
                    │         input.value/output      G5  content
                    │         gen_ai.request.model    G8  model
                    │         usage.*_tokens          G9  tokens
                    └── TOOL  lookup_price            G4  typed

                    root carries the sum              G9  cost on the root
                    every span carries the scope      G6  G7
```

If this repository ships as a package, put `observability/futureagi/` under your own package root instead of the top level.

## Get the Checker

One file, no dependency beyond the OpenTelemetry SDK. Download it into the directory above before Step 1, because Step 1 runs it.
...
<Note>
Nothing substitutes for this file. A checker that skips G1 reports success on keys the collector refused, because every other gate reads a local capture that a broken integration still writes perfectly.
</Note>
...
## Step 2: One Request Has to Arrive as One Tree
...
# Import this once at process start, AFTER whatever loads your .env and before any model
# call. Imported earlier the keys are not there yet, and only G1 says so.
...
The three standard variables, `OTEL_EXPORTER_OTLP_TRACES_ENDPOINT`, `OTEL_EXPORTER_OTLP_TRACES_HEADERS` and `OTEL_RESOURCE_ATTRIBUTES`, are an alternative to the module above.

</Tab>
</Tabs>

## Step 3: One Scope at the Edge Carries Session and User
...
## Step 4: A Field Is Empty Because the Attribute Behind It Was Never Sent

None of these are filled in for you on our side, cost included. Five of the ten gates depend on this step.
...
The instrumentor writes every LLM key for the calls it wraps. The root, a local `TOOL` or `RETRIEVER` span, and cost all fall outside it. A hand-rolled client has no instrumentor at all, so its LLM span is written the same way, by hand, with the model, the messages and the counts taken off the response object.

The kind is the bare name in capitals: `CHAIN` for the root, `LLM` for one model call, then ten more listed in the [instrumentor reference](/docs/tracing/auto).

### Cost is not calculated for you. You send it.

From the token counts and the published rate, onto the root, which is what the trace list reads. It is billed per LLM span as each one ends and added up there, because an ended span is read-only and cannot carry the number itself.

Rates come from the environment, so a price change is configuration rather than a release:
...
<Warning>
A streamed call is missing two of these. Pass `stream_options={"include_usage": True}` and the token counts arrive on the final chunk (G9); without it the provider sends none at all. The model name never arrives either, and the instrumentor's span is never current in your code, so it has to be set as the span opens (G8). Off a stream both arrive on their own.
</Warning>
...
## Step 5: Run It, and Run It Again After Every Change

Three commands. It exits `0`, or it names the gate that failed and why.
...
Nine gates read the local capture in `.fi_verify/spans.jsonl`. G1 reads two receipts written during the run, because a capture says nothing about whether anything arrived. If the same gate fails twice for the same reason, the problem is outside your codebase.

## Step 6: Bind Evals to Attributes the Trace Now Carries

An eval can only grade what it is bound to, and you bind every variable yourself. One with nothing bound reports nothing, which looks the same as one that found no problems.
...
Read your own attributes out of `.fi_verify/spans.jsonl` first, then pick from the [eval catalogue](/docs/evaluation/builtin) and attach them with [Setup evals](/docs/observe/guides/setup-evals).
...
- [ ] `preflight` returns `200` and refuses all three broken variants
- [ ] `project_name` and `project_type` are on the resource, not the span
- [ ] One request produces one trace with one root and no orphans
- [ ] `session.id` and `user.id` are on every span, including the root
- [ ] Every span carries `gen_ai.span.kind`, and at least one is `LLM`
- [ ] `input.value` and `output.value` are on every LLM span
- [ ] `gen_ai.request.model` is on every LLM span, streamed calls included
- [ ] Token counts on the LLM spans and `gen_ai.cost.total` on the root, greater than zero
- [ ] `.fi_verify/` is in `.gitignore` and no credential appears in any span attribute
- [ ] `fi_verify.py check` exits `0`

## Related

<CardGroup cols={2}>
  <Card title="No traces appear" icon="triangle-exclamation" href="/docs/observe/troubleshooting/no-traces-appearing">
    Diagnose an app that runs clean but sends nothing
  </Card>
  ...
  <Card title="Auto-Instrumentation" icon="plug" href="/docs/tracing/auto">
    Setup guides for every supported framework
  </Card>
  <Card title="Setup evals" icon="gauge" href="/docs/observe/guides/setup-evals">
    Attach the evals from Step 6 to your traces
  </Card>
</CardGroup>
