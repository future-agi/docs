...
<a href="https://github.com/future-agi/cookbooks/blob/cookbook/quickstart-notebooks/quickstart/instrument-and-verify.ipynb" target="_blank" style={{display: "inline-flex"}}><img src="https://img.shields.io/badge/View_on_GitHub-181717?logo=github&logoColor=white" alt="GitHub" style={{height: "28px"}} /></a>
</div>
...
Every step below offers two tracks, the FutureAGI SDK and plain OpenTelemetry. **Pick one and stay on it from here to the end.** The two write files of the same name that are not interchangeable, so every listing names its own track on the first line.
...
```python
# observability/futureagi/setup.py     FutureAGI SDK track
import os, sys
...
```python
# your entry point: the scope outside, the root inside     FutureAGI SDK track
...
```python
# observability/futureagi/futureagi_spans.py     FutureAGI SDK track
...
```python
# observability/futureagi/futureagi_rollup.py     FutureAGI SDK track
...
`observability/futureagi/` is the four files from Step 2 and Step 4 on the FutureAGI SDK track, with `traceai-openai-agents` as the instrumentor. The one file added outside it is the entry point the repository does not have. No business logic was edited, and `main.py` was imported, not modified.
...
<img src="/images/docs/cookbook-instrument-and-verify/trace-list.png" alt="The FutureAGI trace list showing two support.turn traces with input, output, latency, tokens, total cost and model" />
...
<img src="/images/docs/cookbook-instrument-and-verify/trace-tree.png" alt="The trace tree for one support turn, from the support.turn root through the triage agent, the handoff, the FAQ agent, the tool span and three LLM spans, with the attributes panel showing session.id, user.id and cost" />

Because `session.id` was set once at the edge, both turns group without either turn knowing about the other.

<img src="/images/docs/cookbook-instrument-and-verify/session.png" alt="The FutureAGI sessions view showing one session with its first and last message, duration, total cost, three traces and the user id" />

<Note>
The model on these captures reads `llama-3.3-70b-versatile` because the run pointed at an OpenAI-compatible endpoint that is not OpenAI. Set `AGENT_MODEL` to whatever you use. Nothing in the integration changes with the provider, and `FI_MODEL_RATES` is where its published rate goes.
</Note>
