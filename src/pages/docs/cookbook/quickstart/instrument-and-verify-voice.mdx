...
*One row per call. The row is the conversation span itself, which is why Step 5 has to open it before `session.start()`: the fifteen LiveKit spans are its children and never appear here on their own.*
...
<img src="/images/docs/cookbook-instrument-and-verify-voice/call-detail.png" alt="The Future AGI voice call detail for the run above: the Voice tab, the transcript with all four turns, and the Call Analytics strip reading 4 turns, 44 words and 0 interrupts" style={{ borderRadius: '5px' }} />
...
<img src="/images/docs/cookbook-instrument-and-verify-voice/call-attributes.png" alt="The Attributes tab of the same call filtered to transcript, showing conversation.transcript, the four numbered conversation.transcript.i.message.role and .content keys, and fi.conversation.transcript" style={{ borderRadius: '5px' }} />
