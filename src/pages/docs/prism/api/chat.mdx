...
<TLDR>
`POST /v1/chat/completions` is the main endpoint. It works exactly like the OpenAI API - same request body, same response format. Prism adds routing, caching, guardrails, and cost tracking transparently. Supports streaming via SSE.
</TLDR>
...
Prism translates the request to each provider's native format automatically. Your code doesn't change.
