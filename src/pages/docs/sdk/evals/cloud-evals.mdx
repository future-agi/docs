...
When you need to check something subjective (is this response helpful? is the tone right? did the model hallucinate?), local heuristics aren't enough. Cloud evals send your data to Future AGI's Turing models for scoring. Templates are managed server-side, so new ones appear without a pip upgrade.
