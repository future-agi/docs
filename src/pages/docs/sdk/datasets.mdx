...
Datasets are structured tables that hold your test data and evaluation results. You create a dataset, add columns and rows, optionally generate LLM outputs, run evaluations, and download the scored results.
...
Generate outputs by running an LLM across your dataset rows. Use `{{column_name}}` to reference column values in your prompt.
...
This creates a new column `gpt4o_response` with the LLM's output for each row.
...
Most methods return `self`, so you can chain the full pipeline:
