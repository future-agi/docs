...
Everything you can do to a [prompt template](/docs/prompt/concepts/understanding-prompts) in the editor, you can also do from code: build it, move it through drafts and versions, point [labels](/docs/prompt/concepts/versions-and-labels) at a version, fetch it by name, and compile it into messages ready for a model call. This page is the call reference for both SDKs.
...
<Note>
Assigning a label to a version that's still a draft doesn't fail: `assign_label()` / `labels().assign()` queue the assignment and apply it automatically on your next commit.
</Note>
...
<CardGroup cols={3}>
  <Card title="Versions & Labels" icon="tags" href="/docs/prompt/concepts/versions-and-labels">
    What a version and a label mean before you call the API for them
  </Card>
