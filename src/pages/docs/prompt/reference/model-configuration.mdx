...
The generation settings below live on the prompt template's model configuration, set from the Playground's editor header or `ModelConfig` in the SDK.
...
A value outside its range is rejected before the model runs. From the Playground, that shows up as the run failing with the error in the output panel (see [Run a prompt](/docs/prompt/guides/run-a-prompt#run-the-prompt-and-read-the-output)); from the SDK, it's a validation error on the call.
...
Limits on how a prompt call behaves once it's running, whether that's one call from the Playground or SDK, or many calls at once from a [Run Prompt column](/docs/dataset/features/run-prompt) across a dataset.
