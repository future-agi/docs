...
<Steps>
  ...
  <Step title="Map its required inputs and name the eval">
    Point each required input at one of support-agent's own variables (`company_name`, `customer_question`), at `model_input`, or at `model_output`, and give the eval a name. Both are required before **Save Eval** works.
  </Step>
  ...
  <Step title="If support-agent doesn't have what the eval needs">
    Pick a different template whose required inputs actually match what support-agent produces.
  </Step>
</Steps>
