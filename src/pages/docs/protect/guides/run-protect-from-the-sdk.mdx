...
Protect reads `FI_API_KEY` and `FI_SECRET_KEY` from your environment; both must be set, or `Protect()` raises an authentication error at construction, before you ever call `protect()`. Generate these on the [API Keys](/docs/admin-settings/api-keys) page if you don't have them yet.
...
For how these checks fit into the guardrail model, see [Protect concepts](/docs/protect/concepts/understanding-protect).
...
If a rule doesn't finish before `timeout` elapses, `protect()` doesn't raise for that: the rule lands in `uncompleted_rules` instead.
