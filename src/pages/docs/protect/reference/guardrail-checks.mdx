...
These 18 checks share a single provider list under **Provider Settings** in the check's Rules dialog (the per-check settings dialog under **Gateway > Guardrails**, covered in [Configuration fields](#configuration-fields)). Provider Settings is where each check's provider configuration lives, not always a credential connection to an outside service.
