---
title: "Skills"
description: "Learn what a skill is, how to turn one on, and use the ones built in."
---

## What a skill is

A **skill** is a saved set of instructions. Turning one on changes how Falcon approaches the request and which [tools](/docs/falcon-ai/concepts/understanding-falcon-ai) it reaches for, including anything connected through [MCP Connectors](/docs/falcon-ai/concepts/mcp-connectors), rather than just answering off a single prompt.
...
For example, typing `/debug-traces` at the start of a message runs the Debug Traces skill directly; if it doesn't match a skill, Falcon treats the text as an ordinary message instead. Every skill also carries at least one trigger phrase, visible on its card in the Customize panel.
