---
title: "Imagine"
description: "AI-powered trace visualization builder. Chat with Falcon AI to turn any trace into an interactive view, then save it to apply to any other trace in the project."
---

## About

Imagine is a custom view builder for traces. You describe what you want to see in plain English, Falcon AI assembles the widgets against the current trace, and the resulting view can be saved and reapplied to any other trace in the same project, including ones that already exist.

A view is a collection of widgets: charts, tables, metric cards, span trees, markdown summaries. Building one by hand means writing custom code or running queries one at a time. Imagine replaces that with chat, then makes the result reusable so the same trace investigation can run on any trace in the project.
...
On a fresh trace, suggested prompt chips appear on the empty canvas: *"Show me latency split by span"*, *"Visualize token usage by span"*, *"Show the agent execution flow"*, *"What's the cost breakdown?"*, *"Compare the model calls"*, *"Summarize this trace"*. Click a chip to send it to the chat panel, or type your own request.
...
<Steps>
  <Step title="Build the view">
    ...
    ![Build a view](/screenshot/product/falcon/11.png)
  </Step>
...
- Each completed widget is cached against the trace ID, so reopening the view on the same trace skips the agent call.
