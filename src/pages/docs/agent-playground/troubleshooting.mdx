...
### A connection I'm drawing gets rejected

Two rules apply when you drag an edge:

- A connection that would loop the graph back on itself is rejected. Route around the loop instead
- An input port that already has an incoming edge won't accept a second one. Delete the existing edge into that input before drawing the new one
