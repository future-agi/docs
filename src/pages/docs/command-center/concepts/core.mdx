---
title: "How it works"
description: "Understand Command Center's request pipeline, plugin architecture, virtual keys, multi-tenancy, and configuration hierarchy."
---
...
Command Center is a proxy that sits between your application and your LLM providers. Every request passes through a chain of plugins before reaching the provider, and the response passes through another chain on the way back.
...
After all pre-request plugins pass, Command Center forwards the request to the selected LLM provider. The routing layer picks the provider based on your configured strategy (round-robin, weighted, least-latency, etc.) and handles failover if the primary provider is down.
...
Command Center uses virtual keys (prefixed `sk-agentcc-`) to authenticate requests. These are not your provider API keys - they're Command Center-specific keys that map to an organization and its configuration.

When a request arrives with a virtual key, Command Center:
...
Your application never sees or stores raw provider API keys. Rotate a provider key in Command Center and every application using that org's virtual keys picks up the change automatically.
...
**Sessions:** Group related requests using the `x-agentcc-session-id` header. Sessions are for grouping and analytics only. Command Center does not maintain conversation state between requests.
...
<CardGroup cols={2}>
  <Card title="Quickstart" href="/docs/command-center/quickstart">
    Get your first request through Command Center in 5 minutes
  </Card>
