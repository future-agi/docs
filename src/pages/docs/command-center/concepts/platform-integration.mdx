...
description: "How Command Center Gateway connects to the broader Future AGI platform — observability, evaluation, protection, and experimentation."
---

## About

Command Center is not a standalone gateway. It's the data collection and enforcement layer of the Future AGI platform. Every request through Command Center generates signals that flow into Observe, Evaluate, Protect, and Experiment — closing the loop between production traffic and model quality.
...
Every request through Command Center generates an **execution trace** — request, response, latency, token counts, cost, provider used, routing decision, and guardrail outcomes. These traces feed directly into the Observe product.
...
The `futureagi` guardrail type connects Command Center to Evaluate:
...
Guardrail scores and decisions are logged in both Command Center (for traffic analysis) and Evaluate (for quality trend tracking).

---

## Command Center → Experiment

Shadow experiments in Command Center generate comparison data that feeds directly into Experiment pipelines.

When you configure traffic mirroring, Command Center collects:
...
The `x-agentcc-metadata` header (or `metadata=` parameter in the SDK) is how you connect Command Center data to your application's dimensions. Tags set on requests flow through to all connected products:
