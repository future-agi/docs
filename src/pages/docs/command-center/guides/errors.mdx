...
description: "Error response format, HTTP status codes, and retry strategies for the Command Center Gateway."
---

## About

All Command Center errors follow a consistent JSON format with machine-readable codes. This page covers the error structure, HTTP status codes, and retry strategies.

---

## Error format

All errors from Command Center follow the same JSON structure:
...
<Tabs items={["Command Center SDK", "OpenAI SDK", "Python (manual)"]}>

<Tab title="Python (Command Center SDK)">

The Command Center SDK retries automatically when you configure `RetryConfig`:
...
For production systems, configure [routing with failover](/docs/command-center/features/routing#failover) instead of client-side retries. Command Center automatically routes to the next provider on failure, which is faster than waiting and retrying the same provider.

---

## Error handling in SDKs

### Command Center SDK exceptions
