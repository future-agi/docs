...
description: "Deploy Command Center Gateway on your own infrastructure using Docker or a Go binary."
---

## About

Command Center is distributed as a Go binary and Docker image. Self-hosting gives you full control over data residency, network topology, and configuration. All requests stay within your infrastructure.

Whether you're running a single instance for development or scaling to production, Command Center handles routing, failover, caching, and rate limiting across multiple LLM providers.

## Requirements

- **Docker** (for container deployment) or **Go 1.23+** (to build from source)
- A publicly routable endpoint (if self-hosted LLM providers need to connect back to Command Center)
