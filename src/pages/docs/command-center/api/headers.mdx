...
description: "Reference for all x-agentcc-* request headers and response headers returned by the Command Center Gateway."
---

## About

Command Center reads `x-agentcc-*` request headers to control per-request behavior (caching, sessions, routing) and writes `x-agentcc-*` response headers to report what happened (which provider, latency, cost, cache status).

The Command Center SDK handles these automatically. If you're using the OpenAI SDK or cURL, set them manually or use `create_headers()` to generate them.
...
| Header | Value | Description |
|---|---|---|
| `x-agentcc-config` | JSON string | Full `GatewayConfig` serialized as JSON. Overrides all per-request settings (cache, retry, fallback, guardrails, routing, timeouts). The Command Center SDK's `GatewayConfig.to_headers()` generates this automatically. |
...
Every response from the Command Center SDK has a `.agentcc` attribute with typed access to all gateway metadata:
