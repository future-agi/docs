...
description: "Use the OpenAI Assistants API through Command Center for managed conversations with tool use and file retrieval."
---

## About

Command Center fully proxies the OpenAI Assistants API. Create assistants with instructions and tools, manage conversation threads, and execute runs - all through the gateway. You get the same Assistants API you'd use with OpenAI directly, plus Command Center's routing, cost tracking, rate limiting, and logging on every call.

The Assistants API is stateful (OpenAI stores threads and messages server-side), so it only works with OpenAI as the provider. Use the OpenAI SDK pointed at Command Center.
...
Since Command Center proxies every Assistants API call, you get:
...
The `x-agentcc-*` response headers are returned on every Assistants API response, just like any other Command Center endpoint.
