---
title: "MCP & A2A"
description: "Connect AI agents to Command Center using the Model Context Protocol (MCP) and Google's Agent-to-Agent (A2A) protocol."
---

## About

Command Center supports two interoperability protocols for AI agents:

- **MCP (Model Context Protocol)**: Agents connect to Command Center to discover and call tools from a unified interface
- **A2A (Agent-to-Agent)**: Agents delegate tasks to other agents through a standardized protocol

Both protocols enable you to build agent networks where Command Center acts as a central hub for tool aggregation and agent coordination.

## MCP — Model Context Protocol

### How Command Center uses MCP

Command Center operates as both an MCP server and an MCP client simultaneously:

- **As an MCP server**: Your AI agents connect to Command Center at `/mcp` to discover and call tools
- **As an MCP client**: AgentCC connects to your upstream tool servers and aggregates their tools into a single namespace

This dual role lets you build a tool mesh where agents see all available tools through Command Center, regardless of where those tools actually run.

```
Agent → /mcp → Command Center → Tool Server A
                    ├→ Tool Server B
                    └→ Tool Server C
```
### Registering MCP servers via the dashboard

Before agents can call tools through Command Center, you need to register the upstream tool servers.
...
### Connecting an agent to Command Center via MCP

Agents connect to Command Center using JSON-RPC 2.0 over HTTP. Start by initializing a session:
...
Command Center supports the following MCP methods:

| Method | Description |
|--------|-------------|
| `initialize` | Start an MCP session with Command Center |
...
Command Center exposes admin endpoints for monitoring and testing MCP:
...
When you create or update an API key, you can specify allowed and denied tool lists. Command Center enforces these restrictions at the MCP layer, so agents using that key will only see and be able to call permitted tools.
...
### How Command Center uses A2A

Command Center acts as an A2A node: it can receive tasks from other agents and delegate tasks to downstream A2A agents. This enables agent-to-agent communication and task delegation without requiring direct connections between agents.
...
Command Center routes the request to the named agent and returns the response. The agent handles the task asynchronously and returns results when ready.

### Listing registered agents

View all downstream A2A agents registered with Command Center:
...
Command Center exposes its own A2A agent card at `/.well-known/agent.json`. This metadata describes Command Center's capabilities, skills, and authentication schemes to other A2A-compatible systems.
...
| Method | Path | Description |
|--------|------|-------------|
| GET | `/.well-known/agent.json` | Command Center's agent card with capabilities and skills |
| POST | `/a2a` | Send a message or task to Command Center as an A2A agent |
...
Command Center's agent card specifies which schemes it supports. When routing to downstream agents, Command Center automatically includes the appropriate credentials.
