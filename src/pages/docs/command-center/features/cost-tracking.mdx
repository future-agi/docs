...
Command Center calculates the cost of every request automatically based on token usage and model pricing. The cost appears in the `x-agentcc-cost` response header and in the `response.agentcc.cost` SDK accessor. No setup required.
...
The Command Center SDK also tracks cumulative cost across all requests made with a client:
...
Go to **Command Center > Settings > Alerts** in the Future AGI dashboard. Create a new alert by selecting the event type, setting recipients, and configuring severity.
