...
Guardrails are safety checks that run on every request and response flowing through Command Center. They catch dangerous or unwanted content before it reaches the LLM (pre-processing) or before it reaches your users (post-processing).
...
Command Center includes 18+ guardrail types covering common safety scenarios.
...
Command Center integrates with leading guardrail and security providers.
...
Choose how Command Center handles guardrail violations.
...
<Tabs items={["Dashboard", "Python (Command Center SDK)", "TypeScript (Command Center SDK)"]}>
  <Tab title="Dashboard">
    ...
    1. Go to **Command Center > Guardrails** in the Future AGI dashboard
