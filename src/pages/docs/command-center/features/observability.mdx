---
title: "Observability"
description: "Monitor Command Center Gateway with logs, metrics, and distributed tracing."
---

## About

Command Center logs every request and response, exports metrics to Prometheus and OpenTelemetry, and propagates trace IDs for distributed tracing. No additional setup needed for basic logging - it's on by default.

---

## Request logging

Every request through Command Center is logged with:
...
Logs sync to the Future AGI dashboard automatically. View them in **Command Center > Logs**.
...
Command Center propagates trace IDs across the request lifecycle. Set `x-agentcc-trace-id` on incoming requests and the same ID appears in all downstream provider calls and logs.

<Tabs items={["Command Center SDK", "OpenAI SDK", "cURL"]}>
...
If you don't set a trace ID, Command Center generates one automatically. Use it for correlating gateway logs with your application logs.
...
Command Center exports Prometheus metrics on the `/-/metrics` endpoint.
...
Sessions appear in the dashboard under **Command Center > Sessions** and show:
