...
Command Center can send real-time HTTP notifications to your endpoints when gateway events occur, such as completed requests, triggered guardrails, exceeded budgets, and errors. Use webhooks to build integrations, trigger alerts, or log events in your own systems.
...
If you set a **Secret**, Command Center signs each request with HMAC-SHA256 and includes the signature in the `X-AgentCC-Signature` header. Verify it on your server to confirm the payload came from Command Center.
