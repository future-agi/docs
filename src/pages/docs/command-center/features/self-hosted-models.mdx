...
description: "Connect Command Center to locally-running models via Ollama, vLLM, LM Studio, and other OpenAI-compatible servers."
---

## About

Command Center can route requests to models running on your own hardware alongside cloud providers. Self-hosted models are configured as providers with a `base_url` pointing to your local inference server. All gateway features (routing, caching, failover, guardrails) work the same way.
...
Ollama auto-discovers all pulled models. After pulling a model (`ollama pull llama3.1`), it's immediately available through Command Center.
...
The main value of self-hosted models through Command Center is hybrid routing: use cheap local models for simple requests and fall back to cloud providers for complex ones.
...
client = AgentCC(
    ...
    base_url="http://localhost:8080",  # your self-hosted Command Center gateway
)
...
<CardGroup cols={2}>
  <Card title="Self-hosted deployment" href="/docs/command-center/deployment/self-hosted">
    Deploy the Command Center gateway on your infrastructure
  </Card>
