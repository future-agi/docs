...
Command Center caches LLM responses server-side. A cache hit returns an instant response without calling the provider. The `X-AgentCC-Cache` response header shows cache status (`hit` or `miss`), and `X-AgentCC-Cost` returns `0` on exact cache hits since no provider tokens were consumed.
...
Go to **Command Center > Caching** in the Future AGI dashboard to enable caching, choose a strategy, and set TTL.
...
<Tabs items={["Python (Command Center SDK)", "Python (OpenAI SDK)", "cURL"]}>
