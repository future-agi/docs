...
**Annotation** is how a person turns their judgement on a piece of AI output (a rating, a category, a correction) into a record Future AGI can compare against every other judgement made the same way. An annotator, someone on your team, works through a queue when there's a batch of sources to get through, or judges a source directly when there isn't. Either path ends at the same four objects: a label, a queue, an item, and a score.
...
- A call execution (a simulated voice or text call, badged Simulation in the UI)
- A prototype run (a prompt run over a dataset, badged Prototype in the UI)
- A dataset row
