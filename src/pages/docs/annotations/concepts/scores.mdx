...
A **score** is one answer, from a person or a system, to one [label](/docs/annotations/concepts/labels) about one [source](/docs/annotations/concepts/understanding-annotation). It's the single record type behind every judgement in Annotation, whether it came from working a [queue item](/docs/annotations/concepts/queues-and-items) directly or from an inline score submitted against a source, which resolves to a queue item of its own.
...
Every score also carries a `score_source` tag recording where it came from: `human` or `api`. The tag also allows for `auto` and `imported` values, but those are origin tags for scores written by other paths, not something you can produce from the app today. In the app UI, it's a record of origin you can read back, not a setting you choose.
