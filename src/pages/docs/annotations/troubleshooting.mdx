...
| Symptom | Cause | Fix |
|---|---|---|
| The submit button won't enable | Every label attached to the queue needs an answer before you can submit | Answer all the labels, the panel names which ones are still open |
| An item says it's reserved by someone else | Another annotator already has the item open | Wait for them to finish or release it |
...
The items stay in the queue. The queue itself moves to the Archived view on the queue list, and any rules attached to it pause. Restore it from the Archived view and it comes back in the status it had when you archived it.
