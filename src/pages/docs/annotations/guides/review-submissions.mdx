...
If you only hold the reviewer role, open the queue from its list and use the **Review Items** button in the header instead.

The toggle switches the workspace you're already in from annotating to reviewing, and the **Review Items** button opens the review workspace directly on the first item pending review.
...
- **Request changes** stays disabled until you've left a note or targeted feedback on at least one answer
...
When the problem is one specific answer rather than the whole item, the comparison panel gives you a feedback field scoped to that answer: **Clear** discards what you've typed, **Done** closes it. That's the targeted alternative to a whole-item note when only one annotator's answer needs fixing. Filling in this field, or the whole-item note, is what enables Request changes; clearing it back out is what re-enables Approve.

## Approve in bulk

When there's nothing to argue with, you don't have to open every item on its own. From the queue's **Items** tab, select the items you want to clear and click **Approve Selected**, which shows the count you've picked.
