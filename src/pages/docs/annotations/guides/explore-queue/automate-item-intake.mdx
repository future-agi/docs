...
1. Give it a name
2. Choose a **Source type** (Dataset Row, Trace, Span, Session, or Simulation) to say what kind of candidate the rule looks at
3. Set the **Trigger**: **Manually**, **Every hour**, **Daily**, **Weekly**, or **Monthly**. Manually means the rule never fires on its own, someone has to run it every time; the other four put it on a recurring schedule instead
4. Pick the specific Dataset, Project, or Agent Definition the rule reads from; if the queue is already scoped to a dataset or project, that field is locked and reads "Locked by this queue"
...
Even without that email, annotators find out. Everyone gets a new-item email at most once an hour, plus a daily summary at their own local digest hour, unless they've unsubscribed. Whatever a rule adds to `Support quality review` reaches annotators on both cadences, so a rule firing while you're not watching still gets to the people who need to work the items.
