...
- This example needs two workspace members picked as annotators in the drawer, since submissions per item can't exceed the number of annotators picked there
- Invite them from [User Management](/docs/admin-settings/user-management) if they're not in your workspace yet
- Working solo? Pick yourself as an annotator in the picker and set submissions per item to 1
</Note>
...
Add the workspace members who'll work this queue as annotators. You become a manager on the queue automatically once you save, but that happens separately from this picker, so it doesn't count toward submissions per item: only the annotators you pick here do. For `Support quality review`, add two members, since the example sets two submissions per item.
