...
<Steps>
  ...
  <Step title="Rebuild the frontend">
    ```bash
...
<Warning>
`VITE_HOST_API` is baked in at build time, not read at runtime. If the browser still calls the old host after you change it, the frontend wasn't rebuilt.
</Warning>
...
Rotate the `CHANGEME` values from the [Checklist](/docs/self-hosting/production/checklist) first, then move them into the manager and inject them at deploy time.
