...
Future AGI provides Python and TypeScript SDKs. The Command Center Gateway also supports direct REST API calls via cURL or any HTTP client.
...
Command Center is Future AGI's AI Gateway. It sits between your application and 100+ LLM providers, handling routing, guardrails, caching, cost tracking, and observability through a single API. See [Command Center Overview](/docs/command-center).

**Do I need to change my code to use Command Center?**

No. If you use the OpenAI SDK, just change `base_url` to `https://gateway.futureagi.com` and swap your API key. See the [Command Center Quickstart](/docs/command-center/quickstart).
