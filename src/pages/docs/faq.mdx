...
Open [Build > Keys](https://app.futureagi.com/dashboard/keys) in the left sidebar. The page is open to Owners, Admins, and Workspace Admins, and all three can add and disable keys. Only an Owner can delete one. If **Keys** is not in your sidebar, your role cannot reach it, so ask an Owner or Admin. See [API Keys](/docs/admin-settings/api-keys).
