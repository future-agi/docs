...
*   Future AGI supports a wide range of evaluations including Hallucination, Guardrails, RAG, etc. See the [Evaluation Overview](/docs/evaluation/running-your-first-eval) for more details.
...
*   You can add documents via the UI or using the SDK. See [Create KB using UI](/docs/knowledge-base/create-kb-using-ui) and [Create KB using SDK](/docs/knowledge-base/create-kb-using-sdk).

**What file types are supported?**

*   PDF, DOCX, TXT, RTF, CSV, JSON, and more. Refer to the [Knowledge Base Concepts](/docs/knowledge-base/overview).

## Dataset

**How can I import data?**

*   Data can be added manually, via file upload, SDK, or imported from Hugging Face. See the [Adding Dataset](/docs/dataset/adding-dataset/manually-creating) section.

**What are dynamic columns?**

*   Dynamic columns allow you to generate new data based on existing columns using prompts, API calls, code execution, etc. Learn more in [Create Dynamic Column](/docs/dataset/dynamic-column/using-run-prompt).

## Experimentation

**What is an Experiment in Future AGI?**

*   Experiments allow you to systematically compare different prompts, models, or parameters. See the [Experimentation Overview](/docs/experimentation/overview).
...
*   Prototyping helps you iterate quickly on different versions of your AI application or prompt. See the [Prototype Overview](/docs/prototype/overview).

**How do I choose a winning prototype?**

*   You can compare prototypes based on evaluations and select the best performing one. See [Choose Winner](/docs/prototype//winner).

## Observe

**What can I monitor with Observe?**

*   Observe helps monitor key metrics like latency, cost, token usage, and evaluation results over time. See the [Observe Overview](/docs/observe/quickstart).

**How do I set up alerts?**

*   Alerts can be configured to notify you about anomalies or issues based on defined thresholds. See [Alerts and Monitors](/docs/observe/evals).
...
*   Protect helps enforce safety rules like toxicity, PII detection, prompt injection, etc., in real-time. See the [Protect Overview](/docs/protect/overview). 