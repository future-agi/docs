...
- **For non-LLM work that has no LLM meaning to capture.** A plain database query or HTTP handler doesn't benefit from traceAI's LLM conventions. Trace it with raw [OpenTelemetry](https://opentelemetry.io/docs/) instead; it still lands in the same trace tree.
...
- **traceAI is not a replacement for OpenTelemetry.** It's complementary — conventions and instrumentors *on top of* OTel. See [OpenTelemetry](https://opentelemetry.io/docs/).
...
<CardGroup cols={2}>
  <Card title="What is OpenTelemetry?" icon="plug" href="https://opentelemetry.io/docs/">
