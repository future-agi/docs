...
Reach for something else when you need to debug one request (use the [trace explorer](/docs/observe/features/llm-tracing)), get notified on a threshold (use [Alerts](/docs/observe/features/alerts)), or break a metric down per end user (use [Users](/docs/observe/concepts/users)).
