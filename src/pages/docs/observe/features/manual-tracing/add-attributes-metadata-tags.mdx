...
- The attribute keys you want to set — prefer the [semantic conventions](/docs/observe/features/manual-tracing/semantic-conventions) so spans stay queryable.
...
| Can't filter by the attribute | Used a custom key the UI doesn't index. | Use a [semantic-convention](/docs/observe/features/manual-tracing/semantic-conventions) key where one exists. |
