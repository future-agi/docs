...
<Mermaid chart={`flowchart TD
  ...
  accDescr: One request named support_agent.run contains an intent-classification LLM span, an order-status tool span with a nested retriever span, and a response-generation LLM span.
  T["Trace: support_agent.run"] --> S1["llm.intent_classification"]
  T --> S2["tool.check_order_status"]
  S2 --> S3["retriever.order_db"]
  T --> S4["llm.response_generation"]`} />
