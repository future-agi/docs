...
- A [prompt column](/docs/dataset/features/run-prompt) to improve
...
Every trial, baseline or variation, holds one candidate prompt and one result per dataset row. A run scores at most 50 dataset rows. The baseline trial runs your original prompt against those rows, each variation trial its own candidate. That row-by-row result is where the actual measurement happens, not the prompt itself.
...
- The average score ranks the trial against every other trial in the run, baseline included
- The run reports the highest average score as the best score, shown alongside the baseline's so you can see whether the optimizer actually moved the number
...
<Mermaid chart={`flowchart TD
  ...
  accDescr: An optimization run fixes a prompt column, an optimizer algorithm, and a model, and takes a set of evals as its objective. It produces a baseline trial and variation trials, each holding a row result per dataset row. Each row result carries a score and a reason per eval, and the mean of those becomes the trial's average score, which ranks the trial and produces a best score measured against the baseline.
  ...
  Avg -->|"ranks trial"| Best["Best score vs. baseline score"]`} />
