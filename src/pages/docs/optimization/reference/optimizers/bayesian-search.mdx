...
The **On-screen label** column is the field name shown when you run this optimizer from the UI; see [Run an optimization](/docs/optimization/guides/run-an-optimization) for the full form walkthrough. All three parameters are required, whether you're submitting the platform request or constructing the class directly; the **Default** column gives the UI's prefilled value and the SDK constructor's fallback when a keyword argument is omitted.
