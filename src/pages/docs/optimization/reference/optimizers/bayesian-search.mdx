...
The **On-screen label** column is the field name shown when you run this optimizer from the UI; see [Run an optimization](/docs/optimization/guides/run-an-optimization) for the full form walkthrough. The **Default** column describes what the platform's optimization request requires, not arguments to the class constructor shown in the code below.

| Parameter | On-screen label | Default | Description |
|---|---|---|---|
| `min_examples` | Min examples | Required on the platform request | Minimum number of few-shot examples to include in a trial. Fewer examples means less context per trial and a cheaper run |
| `max_examples` | Max examples | Required on the platform request | Maximum number of few-shot examples to include in a trial. More examples means more context but a longer, costlier prompt per trial |
| `n_trials` | No.of trials | 10 | Number of configurations the optimizer tries. Raising it searches more configurations at the cost of more evaluator calls |
...
- the `optimize()` arguments shared by every optimizer: `evaluator`, `data_mapper`, `dataset`, and `early_stopping`, plus `initial_prompts` (required on every `optimize()` call)
