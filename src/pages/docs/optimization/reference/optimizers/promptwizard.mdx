...
| Parameter | On-screen label | Constructor default | Description |
|---|---|---|---|
...
| `refine_iterations` | Refined Iterations | 2 | Number of critique-and-refine iterations applied to the best candidates |
| `beam_size` | Beam size | 1 | Number of top-scoring prompts carried forward at each round |
...
<Note>
If you're porting a `beam_size` value from [ProTeGi](/docs/optimization/reference/optimizers/protegi), note its constructor default is 4, versus 1 for PromptWizard. The field name is shared between the two optimizers, but its default behaves differently in each.
</Note>
