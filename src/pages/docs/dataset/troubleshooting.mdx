...
| Add Row, Add Column, Duplicate, or Delete is greyed out | Permission gates all four: duplicating a row needs update access, deleting one needs delete access. Add Row and Add Column are also disabled while the dataset is processing or synthetic generation hasn't finished, and Add Column alone stays disabled until the dataset has at least one row | Check with an org admin about permission, wait for processing or synthetic generation to finish, or add a row first if you're adding a column to an empty dataset |
