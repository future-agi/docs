...
Each column has its own Property editor for exactly this: Min Length and Max Length for text, Value set to Categorical for a list of allowed values, plus custom properties for anything else. Those column properties, not the column's description, are where allowed values and ranges live.
