...
Adding a column extends your dataset with a new field. You can add two kinds:
...
- **Static: store fixed data** – Add columns for labels, IDs, scores, or metadata you enter or import (text, integer, float, boolean, array, JSON).
- **Run Prompt** – Generate text, audio, or images per row using an LLM, TTS, STT, or image model.
- **Vector DB** – Fetch relevant chunks from Pinecone, Qdrant, or Weaviate using a query column.
- **API Call** – Call an external API per row and store the response (or a part of it) in the column.
- **Execute Code** – Run Python (e.g. a `main` function) on each row to compute a new value.
- **Extract JSON** – Pull a specific key's value from a JSON column into a new column.
- **Extract Entities** – Use an LLM and instructions to extract structured info from a text column.
- **Classification** – Assign one of your labels to each row from a text column using a model.
- **Conditional** – Apply if/elif/else logic and run different operations (prompt, retrieval, extract, code, classification, API) per branch.
  ...
  <Tab title="Static Column" icon="table-columns">
    <Steps>
      <Step title="Open Add Columns">
        ...
        ![Add Columns](/screenshot/docs/dataset/add-columns-to-dataset/static//1.png)
      </Step>
      ...
      <Step title="Configure and create">
        ...
        ![Create New Column](/screenshot/docs/dataset/add-columns-to-dataset/static/2.png)
      </Step>
    </Steps>
  </Tab>
  <Tab title="Dynamic Column" icon="wand-magic-sparkles">
    ...
    <Steps>
      <Step title="Open Add Columns and choose Run Prompt">
        ...
        ![Run Prompt](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/1.png)
      </Step>
      <Step title="Name the column and build the prompt">
        ...
        ![Run Prompt](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/2.png)
      </Step>
    ...
    <Tip>
      [Run Prompt in Dataset](/docs/dataset/run-prompt-in-dataset) has the full walkthrough.
    </Tip>
  </Tab>
  <Tab title="Vector DB (Retrieval)" icon="database">
    <Steps>
      ...
      <Step title="Name the column and choose provider">
        ...
        ![Retrieval](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/3.png)
      </Step>
      <Step title="Configure database connection">
        Select the **query column**. Add API key/secret. Set **Index Name**, **Namespace**, **Number of Chunks**, and **Query Key**.
        ![Retrieval](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/4.png)
      </Step>
  ...
  <Tab title="API Call" icon="link">
    <Steps>
      ...
      <Step title="Name the column and set output type">
        ...
        ![API Call](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/5.png)
      </Step>
      <Step title="Configure the API request">
        ...
        ![API Call](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/6.png)
      </Step>
  ...
  <Tab title="Extract JSON" icon="file-code">
    <Steps>
      ...
      <Step title="Name the column and select source">
        ...
        ![Extract JSON](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/7.png)
      </Step>
  ...
  <Tab title="Extract Entities" icon="magnifying-glass">
    <Steps>
      ...
      <Step title="Name the column and select source column">
        ...
        ![Extract Entities](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/8.png)
      </Step>
  ...
  <Tab title="Classification" icon="tags">
    <Steps>
      ...
      <Step title="Name the column and select text column">
        ...
        ![Classification](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/9.png)
      </Step>
  ...
  <Tab title="Conditional" icon="code-branch">
    <Steps>
      ...
      <Step title="Name the column and define branches">
        ...
        ![Conditional](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/10.png)
      </Step>
      <Step title="Choose an operation per branch">
        ...
        ![Conditional](/screenshot/docs/dataset/add-columns-to-dataset/dynamic/11.png)
      </Step>
      <Step title="Test and create column">
