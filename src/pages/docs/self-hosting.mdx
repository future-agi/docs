...
- macOS or Linux (Windows is not supported by `./bin/install`)
- [Docker](https://docs.docker.com/get-docker/) and [Docker Compose](https://docs.docker.com/compose/install/) installed and running
- At least 8 GB RAM available to Docker (Docker Desktop on Mac: Settings → Resources → Memory)
...
The installer sets up `.env`, generates secrets, pulls the published images, starts Docker Compose, and prompts you to create your first user. Once the stack is up, open these in your browser:

| Service | URL |
|---|---|
| Frontend | [http://localhost:3000](http://localhost:3000) |
| Backend API | [http://localhost:8000](http://localhost:8000) |
| AgentCC Gateway | [http://localhost:8090](http://localhost:8090) |
