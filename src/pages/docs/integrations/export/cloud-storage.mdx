...
description: "Archive Command Center Gateway logs to S3, Azure Blob Storage, or Google Cloud Storage as compressed JSONL files."
---

<TLDR>
Connect your cloud storage bucket and Future AGI will archive Command Center Gateway request logs as gzip-compressed JSONL files, partitioned by date and hour. Supports Amazon S3, Azure Blob Storage, and Google Cloud Storage.
</TLDR>

## What this does

This integration archives your Command Center Gateway traffic to cloud object storage for long-term retention or offline analysis. Logs are written as gzip-compressed JSONL files, partitioned by date:
...
You also need **Admin** or **Owner** role in your Future AGI workspace, and the [Command Center Gateway](/docs/command-center) set up and receiving traffic.
...
If the Command Center Gateway had no traffic during a sync window, no files are written. Files are only created when there are logs to archive.
</Accordion>
