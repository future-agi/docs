...
| `categorical` | `{ options: [{label, value}], multi_choice: bool }` | List of selectable options. Set `multi_choice` to `true` to allow multiple selections. |
...
| `thumbs_up_down` | `{}` | No additional settings required. Simple thumbs up/down toggle. |
