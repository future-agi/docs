...
<ApiSection title="Response" status={200} statusText="OK">
  ...
  <ResponseField name="provider" type="string">Voice provider (`vapi`, `retell`, `eleven_labs`, `livekit`, `livekit_bridge`, or `others`), or `null`.</ResponseField>
  ...
  <ResponseField name="livekit_url" type="string">LiveKit server URL. `null` if not configured.</ResponseField>
  <ResponseField name="livekit_api_key" type="string">LiveKit API key. `null` if not configured.</ResponseField>
  <ResponseField name="livekit_agent_name" type="string">Registered LiveKit agent name. `null` if not configured.</ResponseField>
  <ResponseField name="livekit_config_json" type="object">LiveKit room configuration JSON, if set.</ResponseField>
  <ResponseField name="livekit_max_concurrency" type="integer">Maximum concurrent LiveKit sessions. Defaults to `5`.</ResponseField>
