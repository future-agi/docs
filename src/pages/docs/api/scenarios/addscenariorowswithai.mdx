---
title: "Add rows to a scenario using AI"
description: "Initiates an asynchronous task to generate and add a specified number of new rows to a scenario's dataset using AI. A description can be provided to g..."
---

# Add rows to a scenario using AI

Initiates an asynchronous task to generate and add a specified number of new rows to a scenario's dataset using AI. A description can be provided to guide the content generation.

<ApiPlayground
  method="POST"
  endpoint="/simulate/scenarios/{scenario_id}/add-rows/"
  baseUrl="https://api.futureagi.com"
  parameters={[{"name": "scenario_id", "in": "path", "required": true, "description": "The UUID of the scenario to which rows will be added.", "type": "string"}]}
  requestBody={{"numRows": 10, "description": "your-description"}}
/>

## Authentication

All requests require authentication via API keys:

| Header | Description |
|--------|-------------|
| `X-Api-Key` | Your API key |
| `X-Secret-Key` | Your secret key |

## Parameters

### Path Parameters

| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `scenario_id` | string | Yes | The UUID of the scenario to which rows will be added. |

## Request Body

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `numRows` | integer | Yes | Number of rows to generate. Range: 10-100. |
| `description` | string | No | Guides AI content generation for the new rows. |

### Example

```json
{
  "numRows": 10,
  "description": "your-description"
}
```

## Responses

### 202

Accepted. The request to generate rows has been queued and is being processed in the background.

- **message**: string
  Example: `Started generating 5 new rows for scenario`
- **scenario_id**: string
- **dataset_id**: string
- **numRows**: integer
  Example: `5`

### 400

Bad Request. The request is invalid. This can happen if the scenario does not have an associated dataset or if the 'numRows' is out of the valid range (1-100).

### 401

Unauthorized. Authentication credentials were not provided or are invalid.

### 404

Not Found. The scenario with the specified ID could not be found.

### 500

Internal Server Error. An unexpected error occurred while processing the request.

## Code Examples

### cURL

```bash
curl -X POST "https://api.futureagi.com/simulate/scenarios/{scenario_id}/add-rows/" \
  -H "X-Api-Key: YOUR_API_KEY" \
  -H "X-Secret-Key: YOUR_SECRET_KEY" \
  -H "Content-Type: application/json" \
  -d '{"numRows": 10, "description": "your-description"}'
```

### Python

```python
import requests

url = "https://api.futureagi.com/simulate/scenarios/{scenario_id}/add-rows/"
headers = {
    "X-Api-Key": "YOUR_API_KEY",
    "X-Secret-Key": "YOUR_SECRET_KEY",
    "Content-Type": "application/json"
}
data = {
    "numRows": 10,
    "description": "your-description"
}

response = requests.post(url, headers=headers, json=data)
print(response.json())
```

### JavaScript

```javascript
const response = await fetch("https://api.futureagi.com/simulate/scenarios/{scenario_id}/add-rows/", {
  method: "POST",
  headers: {
    "X-Api-Key": "YOUR_API_KEY",
    "X-Secret-Key": "YOUR_SECRET_KEY",
    "Content-Type": "application/json"
  },
  body: JSON.stringify({
    "numRows": 10,
    "description": "your-description"
})
});

const data = await response.json();
console.log(data);
```

