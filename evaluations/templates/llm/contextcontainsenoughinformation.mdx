---
title: Context Contains Enough Information
description: 'Evaluate your LLM models on their ability to provide enough information to resolve a conversation.'
---

Context sufficiency evaluation assesses whether the provided context contains enough information to adequately answer a given query. This evaluation is crucial for ensuring that your system has retrieved sufficient relevant information before generating a response.

### Configuration
The evaluation requires the following configuration:

| Parameter | Description |
|-----------|-------------|
| `model`   | Optional: The model to be used for evaluation |

```python
from fi.evals import ContextSufficiency

context_eval = ContextSufficiency(config={"model": "gpt-4"})
```

### Test Case Setup
The evaluation requires both the query and the context that should contain sufficient information to answer it:

```python
from fi.testcases import LLMTestCase

test_case = LLMTestCase(
    query="What is the capital of France?",
    context="Paris is the capital city of France. It is located in the northern part of the country."
)
```

### Client Setup
Initialize the evaluation client with your API credentials:

```python
from fi.evals import EvalClient

evaluator = EvalClient(
    fi_api_key="your_api_key", 
    fi_secret_key="your_secret_key"
)
```

## Complete Example

```python
from fi.evals import ContextSufficiency, EvalClient
from fi.testcases import LLMTestCase

# Initialize the context sufficiency evaluator
context_eval = ContextSufficiency(config={"model": "gpt-4"})

# Create a test case
test_case = LLMTestCase(
    query="What is the capital of France?",
    context="Paris is the capital city of France. It is located in the northern part of the country."
)

# Run the evaluation
evaluator = EvalClient(fi_api_key="your_api_key", fi_secret_key="your_secret_key")
result = evaluator.evaluate(context_eval, test_case)
print(result)  # Will return Pass if context contains enough information to answer the query
```

The evaluation will return:
- `Pass`: If the context contains sufficient information to answer the query
- `Fail`: If the context lacks necessary information to fully answer the query

This evaluation is particularly useful for:
- Validating RAG (Retrieval-Augmented Generation) systems
- Ensuring context completeness before generating responses
- Identifying information gaps in retrieved context
- Improving context retrieval strategies

### Best Practices
1. Ensure context is relevant to the query
2. Include necessary supporting information
3. Avoid overly verbose or redundant context
4. Maintain context coherence and readability
5. Consider including metadata when relevant