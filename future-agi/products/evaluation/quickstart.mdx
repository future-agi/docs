...
**Future AGI Agents** are our own proprietary models trained on a vast variety of datasets to perform evaluations. These models vary in capabilities and are suited for different use cases:
    ...
    - **PROTECT**: Designed for medium-complexity evaluations focused on safety and content moderation tasks.
    - **PROTECT_FLASH**: Lightweight model for very simple safety evaluations with rapid response times.
    - **TURING_FLASH**: Ultra-fast model for very simple evaluations that require basic understanding with minimal latency.
</Note>
