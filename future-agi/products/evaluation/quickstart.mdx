...
<Note>
Additionally, you can directly use the `evaluate` function to evaluate the test case, if you don't want to chain the functions.

```python
from fi.evals import evaluate
from fi.evals.template import Tone

tone_eval = Tone()
result = evaluate(eval_templates=[tone_eval], inputs=[test_case])
tone_result = result.eval_results[0].metrics[0].value
```
</Note>
