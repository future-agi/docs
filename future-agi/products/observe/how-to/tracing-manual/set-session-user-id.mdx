...
Context manager to add session ID to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the session ID as a span attribute, following the traceAI semantic conventions. Its input, the session ID, must be a non-empty string.
...
Context manager to add user ID to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the user ID as a span attribute, following the traceAI semantic conventions. Its input, the user ID, must be a non-empty string.
