...
Context manager to add metadata to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the metadata as a span attribute, following the traceAI semantic conventions. Its input, the metadata, must be a dictionary with string keys. This dictionary will be serialized to JSON when saved to the OTEL Context and remain a JSON string when sent as a span attribute.
...
Context manager to add tags to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the tags as a span attribute, following the traceAI semantic conventions. Its input, the tag list, must be a list of strings.
...
Context manager to add attributes to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the attributes fields as span attributes, following the traceAI semantic conventions. This is a convenient context manager to use if you find yourself using many of the previous ones in conjunction. 
