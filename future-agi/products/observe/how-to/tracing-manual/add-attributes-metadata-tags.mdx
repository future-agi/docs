---
title: Add Attributes, Metadata and Tags to Span
---
...
Notice that the attributes have a specific prefix operation. When adding custom attributes, it's best practice to vendor your attributes (e.x. mycompany.) so that your attributes do not clash with [semantic conventions](/future-agi/products/observe/how-to/tracing-manual/semantic-conventions).
...
Context manager to add metadata to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the metadata as a span attribute, following the traceAI [semantic conventions](/future-agi/products/observe/how-to/tracing-manual/semantic-conventions). Its input, the metadata, must be a dictionary with string keys. This dictionary will be serialized to JSON when saved to the OTEL Context and remain a JSON string when sent as a span attribute.
...
Context manager to add tags to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the tags as a span attribute, following the traceAI [semantic conventions](/future-agi/products/observe/how-to/tracing-manual/semantic-conventions). Its input, the tag list, must be a list of strings.
...
Context manager to add a prompt template (including its version and variables) to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the prompt template fields as span attributes, following the traceAI [semantic conventions](/future-agi/products/observe/how-to/tracing-manual/semantic-conventions). Its inputs must be of the following type:
...
Context manager to add attributes to the current OpenTelemetry Context. traceAI auto instrumentators will read this Context and pass the attributes fields as span attributes, following the traceAI [semantic conventions](/future-agi/products/observe/how-to/tracing-manual/semantic-conventions). This is a convenient context manager to use if you find yourself using many of the previous ones in conjunction. 
...
Our traceAI core instrumentation package offers a convenience function, `get_attributes_from_context`, to read the context attributes set above from OTEL context.
