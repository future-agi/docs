...
| `contains_all` | Output contains ALL keywords | `output`, `keyword` (list) |
| `contains_any` | Output contains ANY keyword | `output`, `keyword` (list) |
| `contains_none` | Output contains NONE of the keywords | `output`, `keyword` (list) |
...
evaluate("length_less_than", output="short", keyword="100")  # 1.0
evaluate("length_greater_than", output="a longer string", keyword="5")  # 1.0
evaluate("length_between", output="hello world", config={"min": 5, "max": 50})  # 1.0
```
...
Fast security scanners accessible through `evaluate()`. All run locally in &lt;10ms. For the full scanner pipeline API, see the [Guardrails Guide](/sdk-reference/guardrails).
